...
public class BottomBarTab extends FrameLayout {
    ...
    public BottomBarTab(Context context, @DrawableRes int icon, @DrawableRes int iconUneble, CharSequence title) {
        this(context, null, icon, iconUneble, title);
    }

    public BottomBarTab(Context context, AttributeSet attrs, @DrawableRes int icon, @DrawableRes int iconUneble, CharSequence title) {
        this(context, attrs, 0, icon, iconUneble, title);
    }

    public BottomBarTab(Context context, AttributeSet attrs, int defStyleAttr, @DrawableRes int icon, @DrawableRes int iconUneble, CharSequence title) {
        super(context, attrs, defStyleAttr);
        init(context, icon, iconUneble, title);
    }

    int icon;
    int iconUneble;

    private void init(Context context, int icon, int iconUneble, CharSequence title) {
        mContext = context;
        this.icon = icon;
        this.iconUneble = iconUneble;
        ...
        mIcon.setImageResource(iconUneble);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
            mIcon.setImageDrawable(CommonUtils.getDrawable(icon));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.mark_color));
        } else {
            mIcon.setImageDrawable(CommonUtils.getDrawable(iconUneble));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.color_font3));
        }
    }
