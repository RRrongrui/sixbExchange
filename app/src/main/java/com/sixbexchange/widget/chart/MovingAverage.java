...
public class MovingAverage {
    ...
    public static List<Double> rsi(List<KLineBean> input, int period) {
        ...
        // skip first point
        KLineBean previousBar = input.get(0);
        output.add(Double.NaN);

        for (int i = 1, size = input.size(); i < size; i++) {
            final KLineBean priceBar = input.get(i);

            final double gain = priceBar.close.doubleValue() > previousBar.close.doubleValue() ? priceBar.close.doubleValue() - previousBar.close.doubleValue() : 0.0;
            final double loss = previousBar.close.doubleValue() > priceBar.close.doubleValue() ? previousBar.close.doubleValue() - priceBar.close.doubleValue() : 0.0;
