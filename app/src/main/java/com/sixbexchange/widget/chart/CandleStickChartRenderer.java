...
import com.sixbexchange.entity.bean.kline.MsgKlineRemindBEan;
...
public class CandleStickChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    List<MsgKlineRemindBEan> data;
    Paint paint = new Paint();

    public void setData(List<MsgKlineRemindBEan> data) {
        this.data = data;
    }
    ...
    public void drawValues(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            int minx = Math.max(mMinX, 0);
            int maxx = Math.min(mMaxX + 1, dataSet.getEntryCount());
            final int count = (int) Math.ceil((maxx - minx) * mAnimator.getPhaseX()) * 2;

            if (minx < 0 || maxx < 0 || count < 0) {
                break;
            }
            ...
            //float textSize = Utils.convertPixelsToDp(v)/2;
            ...
            if (maxIndex > minIndex) {
                ...
                c.drawText(highString, x + highStringWidth / 2,
                        tPosition[1]+highStringHeight/2, mValuePaint);
            } else {
                //画左边
                String highString = BigUIUtil.getinstance().bigPrice(minValue+"")//Float.toString(minValue)
                        + " →";
